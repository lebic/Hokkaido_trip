  ...
  private async drawRoute(): Promise<void> {
    ...
    try {
      ...
      this.zone.runOutsideAngular(() => {
        ...
        L.polyline(latlngs, { color: '#3b82f6', weight: 4, opacity: 0.85 }).addTo(this.routeLayer!);
        this.fitBounds();
      });
      ...
      this.zone.runOutsideAngular(() => {
        const latlngs = this.waypoints.map(wp => [wp.lat, wp.lng] as [number, number]);
        L.polyline(latlngs, { color: '#3b82f6', weight: 3, dashArray: '6 6' }).addTo(this.routeLayer!);
        this.fitBounds();
      });
      this.zone.run(() => {
  ...
  private addMarkers(): void {
    ...
    this.waypoints.forEach((wp, i) => {
      ...
      const stopBadge = isStop
        ? '<br><span style="font-size:11px;color:#92400e">Arrêt suggéré</span>'
        : '';
      const icon = L.divIcon({
        className: '',
        html:
          '<div style="width:28px;height:28px;background:' + bg +
          ';border:2.5px solid white;border-radius:50%;display:flex;align-items:center;' +
          'justify-content:center;font-size:10px;font-weight:700;color:white;' +
          'box-shadow:0 2px 6px rgba(0,0,0,0.35);cursor:default;">' + label + '</div>',
        iconSize: [28, 28],
        iconAnchor: [14, 14],
        popupAnchor: [0, -16],
      });
      L.marker([wp.lat, wp.lng], { icon })
        .bindPopup('<strong style="font-size:13px">' + wp.name + '</strong>' + stopBadge)
        .addTo(this.routeLayer!);
    });
  }
